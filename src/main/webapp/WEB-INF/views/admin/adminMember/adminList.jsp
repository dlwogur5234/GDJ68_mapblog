				...
				<nav aria-label="Page navigation example">
				  <ul class="pagination">
				    ...
				    <li class="page-item ${pager.pre?'':'disabled'}" >
				      <a class="page-link" href="./adminList?page=${pager.startNum-1}&kind=${param.kind}&search=${param.search}" aria-label="Previous">
				        <span aria-hidden="true">&laquo;</span>
				      </a>
				    </li>
				    
				    <c:forEach begin="${pager.startNum}" end="${pager.lastNum}" var="i">
				    <li class="page-item"><a class="page-link" href="./adminList?page=${i}&kind=${param.kind}&search=${param.search}">${i}</a></li>
				    </c:forEach>
				    <c:if test="${pager.next}">
				    <li class="page-item" >
				      <a class="page-link" href="./adminList?page=${pager.lastNum+1}&kind=${param.kind}&search=${param.search}" aria-label="Next">
				...
				<div class="input-group mb-3">
				 <form action="./adminList" method="get">
					  <select name="kind" class="form-select" aria-label="Default select example">
						  <option value="adminGrade">Grade</option>
